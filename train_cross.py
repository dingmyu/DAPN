...
def train(config):
    ...
    fsl_train_target_sampler = CategoriesSampler(dsets["target"].label, 100,
                                      config["train_way"], config["shot"] + config["query"])
    dset_loaders["fsl_target"] = DataLoader(dataset=dsets["target"], batch_sampler=fsl_train_target_sampler,
                              num_workers=4, pin_memory=True)
    dset_loaders["da_source"] = DataLoader(dsets["source"], batch_size=config["shot"] * config["train_way"], \
            shuffle=True, num_workers=4, drop_last=True)
    ...
    for i in range(config["num_iterations"]):
        ...
        optimizer.zero_grad()
        if i % (len_train_source + len_train_fsl_target) == 0:
            start = i
            iter_fsl_train = iter(dset_loaders["source"])
        if i != 0 and (i - start) % len_train_source == 0:
        ...
        # classifier_loss = nn.CrossEntropyLoss()(outputs_source, labels_source)
            ...
            proto = proto_target.reshape(config["shot"], config["train_way"], -1).mean(dim=0)
...
if __name__ == "__main__":
    ...
    parser.add_argument('--s_dset_path', type=str, default='mini-imagenet/train', help="The dataset path")
